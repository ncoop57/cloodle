...
class CloodlesListRoute extends StatelessWidget {
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            new Column(
              children: <Widget>[
                _displayCloodlesReceivedList(),
              ],
            ),
            new Column(
              children: <Widget>[
                _displayCloodlesSentList(),
              ],
            ),
          ],
        ),
  ...
  Widget _displayCloodlesReceivedList() {
  ...
  Widget _displayCloodlesSentList() {
    var _db = Firestore.instance
        .collection("sessions")
        .where("FROM", isEqualTo: currentUser.notification_token);
    return new StreamBuilder(
      stream: _db.snapshots(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) return const Text('Loading...');
